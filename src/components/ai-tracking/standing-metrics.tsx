...
import type { StandingMetric } from "./types"

export function StandingMetricCard({ metric }: { metric: StandingMetric }) {
  return (
    <Card className={`${trackingCardClass} flex h-full flex-col p-0`}>
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
      {standingMetrics.map((metric) => (
        <StandingMetricCard key={metric.id} metric={metric} />
      ))}
    </div>
  )
}

export { standingMetrics }
