...
interface UpNextCardProps {
  data: UpNextData
  /** Nested under Seasonal — skip outer reveal chrome. */
  embedded?: boolean
}

export function UpNextCard({ data, embedded = false }: UpNextCardProps) {
  const { items } = data
  ...
  const card = (
    <Card
      className={cn(
        "relative w-full overflow-hidden rounded-3xl border-0 bg-brand-25 py-0 ring-1 ring-brand-200/70 !shadow-pane-lg",
        embedded && "rounded-2xl !shadow-md",
      )}
    >
  ...
  return <RevealItem className="w-full min-w-0">{card}</RevealItem>
}
